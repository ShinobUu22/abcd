...
if(isset($_POST['submit']))
{
    ...
    $sql = "SELECT * from pms where id=$from";

    $query = mysqli_query($conn,$sql);
    $sql1 = mysqli_fetch_array($query); // returns array or output of user from which the amount is to be transferred.

    $sql = "SELECT * from pms where id=$to";
    $query = mysqli_query($conn,$sql);
    ...
    if (($amount)<0)
   {
    ...
    // constraint to check zero values
         ...
         echo "</script>";
                ...
                $sql = "UPDATE pms set Balance=$newBalance where id=$from";
                mysqli_query($conn,$sql);


                // adding amount to reciever's account
                $newBalance = $sql2['Balance'] + $amount;
                $sql = "UPDATE pms set Balance=$newBalance where id=$to";
                mysqli_query($conn,$sql);
...
<body style="background-color :#E6E6FA;">
  ...
  <a class="navbar-brand"  style="color:black"><b>Himalaya Bank</b></a>
  ...
  <div class="collapse navbar-collapse" id="collapsibleNavbar">
   ...
   <div style="width:90%; margin:0px 60px 0px 50px;">
       ...
       <section id="contact" style="margin-top:30px; color:black;">
          <p>© Copyright 2022 Himalaya Bank</p>
        </section>
